<!--2019-05-16 20:00-->
# Vue.js系列 - Render的使用
> 在Vue中，绝大多数情况下使用模板来创建HTML, 在一些场景中需要使用渲染函数来达到目的
...
当浏览器读到这些代码时，它会建立一个“DOM 节点”树来保持追踪所有内容
...
在代码：`return createElement('h1', this.blogTitle)`中`createElement`返回什么呢？

其实不是一个*实际的* DOM 元素。它更准确的名字可能是 `createNodeDescription`，因为它所包含的信息会告诉 Vue 页面上需要渲染什么样的节点，包括及其子节点的描述信息。
我们把这样的节点描述为“虚拟节点 (virtual node)”，也常简写它为“**VNode**”。
“虚拟 DOM”是我们对由 Vue 组件树建立起来的整个 VNode 树的称呼。

### `createElement` 参数

在 `createElement` 函数中使用模板中的功能, 接受的参数：
